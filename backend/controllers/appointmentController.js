...
import {clerkClient} from "@clerk/clerk-sdk-node" // it gives total number of user data registerd to the  site.
dotenv.config();
...
export const getAppointments = async(req,res) => {
    try{
        //per particular page 50 doctors list is the max shown in the page.
...
export const createAppointment = async(req,res)=>{
    try{
        ...
        const exisitingBooking =await Appointment.findOne({
            doctorId,
        ...
        notes: notes || "",
        createdBy: clerkUserId,//FOR PARTICULAR USER ID.
        owner: resolvedOwner,
        sessionId: null,
        };
        ...
        //FOR ONLINE PAYMENT WE ARE GOING TO USE STRIPE.
        // Online: Stripe
...
export const confirmPayment = async(req,res)=>{
    try{
        ...
        if(session.payment_status !== "paid"){
            return res.status(400).json({
                success:false,
                message:"Payment nnot completed"
            })
        }
