...
//parse the date with time.
function parseDateTime(date, time) {
  return new Date(`${date}T${time}:00`);
}

//the format to get AM or PM
...
//to convert the 24hrs time to 12 hrs
...
//this fucntion will give the details coming from the server.
...
export default function DashboardPage({ apiBase }) {
  ...
  //computes the vaalue for filter.
  useEffect(() => {
    fetchAppointments();
  }, [API, doctorId]);
  ...
  //to reschedule the appointment and also 
  // check whether appointment is cancel or completed then we cannot change.
        ...
        <div className={dashboardStyles.headerContainer}>
          <div>
            ...
            <p className={dashboardStyles.headerSubtitle}>
              {doctorId
                ? `Showing appointments for Dr. ${doctorId}`
